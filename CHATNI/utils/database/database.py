...
async def set_assistant(chat_id):
    from CHATNI.core.userbot import assistants
...
async def add_served_user_clone(user_id: int):
    is_served = await is_served_user_clone(user_id)
    if is_served:
        return
...
async def add_served_chat_clone(chat_id: int):
    is_served = await is_served_chat_clone(chat_id)
    if is_served:
        return
...
async def get_served_users_clone2() -> list:
    return [user async for user in usersdbc.find({"user_id": {"$gt": 0}})]

async def get_served_chats_clone2() -> list:
